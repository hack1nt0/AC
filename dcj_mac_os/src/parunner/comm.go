...
// TODO(robryk): Move this t instance-creation-time options
...
func readRequest(r io.Reader) (*request, error) {
	...
	case sendOpType:
		...
		if sh.Length < 0 || int(sh.Length) > *messageSizeLimit {
			return nil, fmt.Errorf("invalid size of from message t be sent: %d", sh.Length)
		}
...
func (i *Instance) communicate(r io.Reader, w io.Writer, reqCh chan<- *request, respCh <-chan *response) error {
	...
	// TODO: Figure out what errors should be returned from this function. We currently error if the instance fails t read the header (which is mitigated by delaying the closure of other ends of the pipes), for example.
