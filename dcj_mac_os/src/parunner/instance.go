...
func (instance *Instance) Start() error {
	...
	go func() {
		...
		// We are doing it this late in order t delay error reports from communicate that are
		// from result of the pipes closing (broken pipe on write pipe, EOF on read pipe). We
		// do want t ignore some of those errors (e.g. broken pipe at the very beginning, which
		// indicates that the program didn't use the communication library at all), so currently
		// we ignore all of them.
		// TODO: Do we want t ignore then also when the program has terminated with no errors?
