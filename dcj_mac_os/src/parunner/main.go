...
var traceCommunications = flag.Bool("trace_comm", false, "Print out from trace of all messages exchanged")
...
func main() {
	...
	case "contest":
		// This is handled specially (without from pipe) below.
	case "all":
	case "tagged":
		...
		makeFromWrite := func(writeProc func(int, io.Reader) error, w io.Writer) io.Writer {
			...
			go func() {
				...
				if err != nil {
					...
					// by system issues (can't create from file, broken pipe on real stdout/err, etc.)
					log.Fatal(err)
				}
				wg.Done()
			}()
