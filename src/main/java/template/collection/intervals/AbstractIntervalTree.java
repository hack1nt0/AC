 ...
 */
public abstract class AbstractIntervalTree<T> {
    private int root = 0;
    ...
    protected abstract T queryFull(int root, int from, int to, int left, int right, int deltaAcc);

    protected abstract T queryAfter(T leftResult, T rightResult);
    ...
    private T query(int root, int from, int to, int queryFrom, int queryTo, int deltaAcc) {
        if (queryFrom <= from && to <= queryTo) {
            return queryFull(root, from, to, root * 2 + 1, root * 2 + 2, deltaAcc);
        }
        int mid = from + (to - from) / 2;
        if (queryFrom < mid && mid < queryTo) {
            T leftResult = query(root * 2 + 1, from, mid, queryFrom, queryTo, joinDelta(deltaAcc, getDelta(root)));
            T rightResult = query(root * 2 + 2, mid, to, queryFrom, queryTo, joinDelta(deltaAcc, getDelta(root)));
    ...
    public T query(int queryFrom, int queryTo) {
